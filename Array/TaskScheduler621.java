	...
	public int countIntervals( Map<Character, Integer> hm, int n, int interval, int totalTasks) {
		...
		while(hm.size() != 0) {
			...
			while (it.hasNext()) {
				...
				if(hm.get(pair.getKey()) == 0) hm.remove(pair.getKey());
//				System.out.println(pair.getKey() + " = " + pair.getValue() + " = " + interval);
				interval++;
				n--;
			}
			if(hm.size() == 0) {
			...
			if(n >= 0) {
				interval = interval + n;
				if(nn >= hmsize) interval = interval + 1;
			}
			
			if(hm.size() == nn) interval++;
