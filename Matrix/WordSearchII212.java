...
//		System.out.println(cn.findWords(board, new String[] {"oath","pea","eat","rain", "rain", "rat"}));
...
//		System.out.println(cn.dfs(board, "aba", 0, 0, 0, new HashSet<String>()));
	...
	public List<String> findWords(char[][] board, String[] words) {
		...
		if(initials.isEmpty() || board.length == 0 || board[0].length == 0) return res;
	...
	public boolean dfs(char[][] board, String word, int i, int row, int col, Set<String> seen) {
		if(!isValid(board, word, i, row, col, seen)) return false;
		
		seen.add(row + "-" + col);
		
		if(board[row][col] == word.charAt(i) && i == word.length() - 1) return true;
		if(dfs(board, word, i + 1, row + 1, col, seen)) return true;
		if(dfs(board, word, i + 1, row - 1, col, seen)) return true;
		if(dfs(board, word, i + 1, row, col + 1, seen)) return true;
		if(dfs(board, word, i + 1, row, col - 1, seen)) return true;
	...
	public boolean isValid(char[][] board, String word, int i, int row, int col, Set<String> seen) {
		if(row < 0 || col < 0 || row > board.length - 1 || col > board[0].length - 1 || seen.contains(row + "-" + col) || i > word.length() - 1 || board[row][col] != word.charAt(i)) return false;
		return true;
	}
}
